...
// Note: These are NOT constexpr because std::bitset::test is not constexpr-compatible
// with MSVC's evaluation rules for constexpr functions. They are called at runtime only.
inline bool b0(const Bit9State& s) noexcept { return s.test(0); }
inline bool b1(const Bit9State& s) noexcept { return s.test(1); }
inline bool b2(const Bit9State& s) noexcept { return s.test(2); }
inline bool b3(const Bit9State& s) noexcept { return s.test(3); }
inline bool b4(const Bit9State& s) noexcept { return s.test(4); }
inline bool b5(const Bit9State& s) noexcept { return s.test(5); }
inline bool b6(const Bit9State& s) noexcept { return s.test(6); }
inline bool b7(const Bit9State& s) noexcept { return s.test(7); }
inline bool b8(const Bit9State& s) noexcept { return s.test(8); }
